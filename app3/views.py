...
from app3.serializers import EmployeeModelSerializer, EmployeeDeserializer
...
class StudentView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        u_id = kwargs.get('pk')
        if u_id:
            user = Students.objects.get(pk=u_id).values("studentname", "password", "gender").first()
            if user:
                return JsonResponse({
                    "status": 200,
                    "message": "获取用户成功",
            ...
            users = list(Students.objects.all().values("studentname", "password", "gender"))
            if users:
                return JsonResponse({
                    "status": 201,
                    "message": "获取用户列表成功",
    ...
    def post(self, request, *args, **kwargs):
        try:
            user_obj = Students.objects.create(**request.data.dict())
            print(user_obj)
            if user_obj:
                return JsonResponse({
                    "status": 200,
